...
$u = 'https://en.m.wiktionary.org/wiki/' . urlencode($q10);
...
if (stripos($headers[0], '200 OK') > 0) {
    ...
    $x = str_replace('>Open main menu<', '><', $x);
    $x = str_replace('<span class="mw-editsection-bracket">]</span>', '', $x);
    $x = str_replace('href="/', 'href="https://en.m.wiktionary.org/', $x);
    $x = str_replace('href=', 'rel="nofollow" href=', $x);
    //$x = str_replace('https://en.m.wiktionary.org/wiki/', 'http://en.webot.org/?search=', $x);
    //$x = str_replace('http://en.webot.org/?search=File:', 'https://en.m.wiktionary.org/wiki/File:', $x);
    ...
    if ($x) {
        echo "<p><cite><small>From Wiktionary</small></cite></p>$x";
    }
}
?>
